...
BOOL ShowStackActiveSpells(H3CombatCreature *stack, bool isRMC, H3DlgItem *clickedItem)
{
    ...
    else if (clickedItem != nullptr)
    {
        ...
        IntAt((int)dlg + 0x18) =
            Clamp(0, xPos, H3GameWidth::Get() - width - 200); // 200 is width of adventure bar on right
        IntAt((int)dlg + 0x1C) =
            Clamp(0, yPos, H3GameHeight::Get() - height - 48); // 48 is height of resource bar on bottom
    }
    dlg->RMB_Show();
    ...
    if (0 && description)
    {
        ...
        if (!Era::IsCommanderId(dlg->creatureId))
        {
            ...
            if (auto *pcx = H3LoadedPcx16::Load(n_SkillSlotPcx))
            {
                ...
                {
                    if (H3DlgPcx16 *creature_skill_slot = H3DlgPcx16::Create(
                            x + i * skill_width - 4, y - 6, skill_width, skill_height, -1, pcx->GetName()))
                    {
                ...
                if (creature_skills_number > SKILLS_VIEW_LIMIT)
                {
                    // scroll
                    scroll_height = 16;
                    auto *scroll = H3DlgScrollbar::Create(
                        x - 4, y + skill_height - 6, skill_width * SKILLS_VIEW_LIMIT, scroll_height, 2020,
                        creature_skills_number - dlgSkillPcx.size() + 1, CreatureDlgSrollbar_Proc, false, 1, true);
                    dlg->AddItem(scroll);
                }
...
BOOL CreatureDlgHandler::AddExperienceButton()
{
    ...
    if (!isNPC || stack != nullptr)
    {
        ...
        H3DlgDefButton *bttn = H3DlgDefButton::Create(x_pos, y_pos, WOG_CREATURE_EXP_BUTTON_ID, "CrExpBut.def", 0, 1,
                                                      false, h3::eVKey::H3VK_E);
        H3String hint = isNPC ? Era::tr("gem_plugin.combat_dlg.creature_info.npc_hint")
                              : Era::tr("gem_plugin.combat_dlg.creature_info.stack_exp_hint");
...
BOOL CreatureDlgHandler::AddSpellEfects()
{
    ...
    for (INT32 i = 0; i < spellsToShow; i++)
    {
        ...
        if (i == 4 && needToExpnd)
        {
            H3DlgDefButton *dlgCallBttn =
                H3DlgDefButton::Create(283, yPos + 42, DLG_SPELLS_BTTN_ID, spellListBtn, 0, 1, false, eVKey::H3VK_S);
...
H3CreatureInfoDlg *__stdcall H3CreatureInfoDlg_NotBattleCtor(HiHook *h, H3CreatureInfoDlg *dlg, H3Army *army,
                                                             int slotId, const DWORD hero, const H3Town *town, int x,
                                                             int y, const int monGrade, const int a9, const int a10,
                                                             const int a11)
{

    auto result = THISCALL_11(H3CreatureInfoDlg *, h->GetDefaultFunc(), dlg, army, slotId, hero, town, x, y, monGrade,
                              a9, a10, a11);
...
void Dlg_CreatureInfo_HooksInit(PatcherInstance *pi)
{
    ...
    {
        int pluginHookAddress =
            wndPlugin.NeedleSearch<6>({0x0F, 0x8C, 0xC8, 0xFE, 0xFF, 0xFF}, 6); // Thanks so much to RK for lesson
    ...
    // pi->WriteWord(0x5F3E10, 0x9090); //allow Faerie Dragon description at LMC in combat creature info dlg -- failed,
    // so added it in handler
    ...
    pi->WriteByte(0x5F4466,
                  0x51); // push ECX ==1 instead EDI == -1 to set as description itemId  in non combat creature info dlg
